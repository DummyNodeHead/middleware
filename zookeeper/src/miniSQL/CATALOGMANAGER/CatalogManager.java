...
public class CatalogManager {
    ...
    public static String getTablesRemote() throws IOException {
        Table tmpTable;
        Attribute tmpAttribute;
        Iterator<Map.Entry<String, Table>> iter = tables.entrySet().iterator();
        ...
        while (iter.hasNext()) {
            Map.Entry entry = iter.next();
            tmpTable = (Table) entry.getValue();
            buf.append(tmpTable.tableName);
            for (int i = 0; i < tmpTable.attributeNum; i++) {
                tmpAttribute = tmpTable.attributeVector.get(i);
                buf.append(" " + tmpAttribute.attributeName);
                buf.append(" " + tmpAttribute.type.get_type().name());
                buf.append(" " + tmpAttribute.type.getLength());
                buf.append(" " + (tmpAttribute.isUnique ? 1 : 0));
            }
            buf.append("\n");
        }
    ...
    public static String getIndexesRemote() throws IOException {
        Index tmpIndex;
        Iterator<Map.Entry<String, Index>> iter = indexes.entrySet().iterator();
        StringBuffer buf = new StringBuffer();
        ...
        while (iter.hasNext()) {
            Map.Entry entry = iter.next();
            tmpIndex = (Index) entry.getValue();
            buf.append(tmpIndex.indexName);
            buf.append(" " + tmpIndex.tableName);
            buf.append(" " + tmpIndex.attributeName + "\n");
        }
    ...
    public static void setRowNum(String tableName, int rowNum) {
        getTable(tableName).rowNum = rowNum;
    }
