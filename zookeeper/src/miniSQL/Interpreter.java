...
public class Interpreter {
    ...
    // private static Lock lock = new ReentrantLock();  // controls concurrency
    ...
    public static void main(String[] args) {
        try {
            ...
            while (true) {
                System.out.println(interpretRemote(reader.readLine()));
                // interpret(reader);
            }
        } catch (IOException e) {
    ...
    public static String interpretRemote(String sql) throws IOException, QException {
        // 去掉最后的分号，否则有bug
        ...
        try {
            ...
            switch (tokens[0]) { // match keyword
                ...
                //     break;
                // case "reset":
                //     CatalogManager.setRowNum(tokens[1], Integer.parseInt(tokens[2]));
                //     break;
                case "create":
                    if (tokens.length == 1)
                ...
                case "drop":
                    ...
                    break;
                case "select":
                    resultValue += parseSelectRemote(result);
                    break;
                case "insert":
                    resultValue += parseInsert(result);
                    break;
                case "delete":
                    resultValue += parseDeleteRemote(result);
                    break;
                case "show":
                    resultValue += parseShowRemote(result);
                    break;
                case "quit":
...
//                        System.out.print("-->"); //next line
    ...
    private static String parseShowRemote(String statement) throws Exception {
        String type = Utils.substring(statement, "show ", "").trim();
        if (type.equals("tables")) {
            return CatalogManager.getTablesRemote();
        } else if (type.equals("indexes")) {
            return CatalogManager.getIndexesRemote();
        } else throw new QException(0, 323, "Can not find valid key word after 'show'!");
    }
    ...
    private static String parseSelectRemote(String statement) throws Exception {
        //select ... from ... where ...
        String attrStr = Utils.substring(statement, "select ", " from");
        String tabStr = Utils.substring(statement, "from ", " where");
        String conStr = Utils.substring(statement, "where ", "");
        ...
        if (attrStr.trim().equals("*")) {
            ...
            if (tabStr.equals("")) {  // select * from [];
                tabStr = Utils.substring(statement, "from ", "");
                Vector<TableRow> ret = API.select(tabStr, new Vector<>(), new Vector<>());
                result += Utils.getRaw(ret, tabStr);
            } else { //select * from [] where [];
                String[] conSet = conStr.split(" *and *");
                //get condition vector
                conditions = Utils.createConditon(conSet);
    ...
    private static String parseDeleteRemote(String statement) throws Exception {
...
class Utils {
    ...
    public static String getRaw(Vector<TableRow> tbl, String tableName) throws IOException {
        StringBuffer buf = new StringBuffer();

        if (tbl.size() == 0) {
            return "";
        } else {
            int numAttrs = tbl.get(0).getAttributeSize();
            for (int i = 0; i < numAttrs; i++) {
                if (i == 0) {
                    buf.append(CatalogManager.getAttributeName(tableName, i));
                } else {
                    buf.append(" " + CatalogManager.getAttributeName(tableName, i));
                }
                buf.append("\n");
            }
            for (TableRow row: tbl) {
                for (int i = 0; i < row.getAttributeSize(); i++) {
                    if (i == 0) {
                        buf.append(row.getAttributeValue(i));
                    } else {
                        buf.append(" " + row.getAttributeValue(i));
                    }
                    buf.append("\n");
